...
function parseModelContent(response) {
  ...
  if (!content) throw new NutritionCoachError('AI_INVALID_RESPONSE', 'DeepSeek response content is empty')

  const jsonText = String(content).trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '')
  try {
    return JSON.parse(jsonText)
  } catch (error) {
    throw new NutritionCoachError('AI_INVALID_RESPONSE', 'DeepSeek response content is not JSON')
  }
}
...
function createDeepSeekProvider(options) {
  ...
  return {
    ...
    async generate(input) {
      ...
      if (!apiKey) throw new NutritionCoachError('AI_KEY_MISSING', 'DEEPSEEK_API_KEY is not configured')
