...
assert.match(todayJs, /inputDockStyle/)
...
assert.match(todayWxss, /--native-tabbar-height:\s*112rpx/)
assert.match(todayWxss, /\.input-dock\s*\{[\s\S]*?position:\s*fixed;[\s\S]*?right:\s*0;[\s\S]*?left:\s*0;[\s\S]*?z-index:\s*999;/)
assert.match(todayWxss, /\.today-page\s*\{[\s\S]*?padding:[^;]+var\(--native-tabbar-height\)[^;]+var\(--composer-reserve\)/)
